...
// `usePolling` against a real QueryClient: a poll must still fire when the
// component observing it re-renders faster than the poll interval.
...
describe("usePolling", () => {
  it("keeps polling while the observer re-renders every second", async () => {
    ...
    renderHook(
      () => {
        // A countdown: one render per second, well inside the 10s interval.
    ...
    // Two intervals at their longest jitter (+20%), plus slack, one rendered
    // second at a time.
    for (let s = 0; s < 25; s++) {
      await act(async () => {
