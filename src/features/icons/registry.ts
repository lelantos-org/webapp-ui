...
// ERC-20 has no `icon()`. Sourcing logos from a CDN at runtime would require
// widening the `img-src 'self' data:` CSP in `index.html` to that host and would
// disclose which assets a user holds. Bundled artwork costs no request and works
// offline in the PWA.
//
// Every entry here is an enhancement rather than a requirement: a missing one
// degrades to a coloured monogram, and nothing downstream branches on whether a
...
interface Brand {
  ...
  art: ReactElement;
  /// Brand colour as HSL, for surfaces that need one, such as the skeleton. Kept
  /// beside the artwork so the two cannot drift.
  color: Hsl;
}
...
/// relayer is deployed against. A chain absent here, including the local anvil
/// stack at 31337, still renders with a hue derived from its id.
...
///
/// Not `(chainId, address)`: USDC is USDC on all six chains, so an address-keyed
/// table would need six rows per token and would stop matching once a deployment
/// registered the same asset from a different bridge. The symbol comes from the
/// token's own `symbol()` via the indexer.
///
/// The trade-off is that a token impersonating a symbol borrows its colour. The
/// colour is decorative — the address is shown and verified elsewhere, and no
/// flow keys off it.
const TOKENS: ReadonlyMap<string, Brand> = new Map([
  // WETH takes ether's mark. `chains.ts` derives `isWeth` from the same symbol,
  ...
  // Gas tokens of the served chains, which a deployment may register as ERC-20s.
  // They reuse the network artwork rather than carrying a second copy.
...
/// whose `symbol()` the indexer has not resolved, which matches nothing.
