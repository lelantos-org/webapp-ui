...
// Crypto, FMD encoding and key derivation, all published from `./primitives`.
...
describe("maxDetectionGamma", () => {
  ...
  // The server's own figure for the live pool ("gamma must be <= 1 at the
  // current note count", so [128, 256)), then one γ per doubling — written out,
  // since the sweep above compares against a transcription.
