...
export function walletDb(): Promise<IDBPDatabase<WalletSchema>> {
  dbp ??= openDB<WalletSchema>(DB_NAME, VERSION, {
    upgrade(db, oldVersion) {
      // Reaching v4 from anything earlier discards the stores rather than
      // migrating them.
      //
      ...
      // wrong root and a rejected spend rather than as an error. Everything
      // here is a cache of chain state, so the cost of dropping it is one
      // resync.
