...
import { closeDepositStreamsExcept } from "@/features/tx";
...
const buildKey = (chainId: bigint, accountKey: string) =>
  `${chainKey(chainId)}:${accountKey.toLowerCase()}`;
...
/// `value` when it was produced for exactly this `(chainId, accountKey)`.
function currentFor<T>(
  built: BuiltFor<T> | undefined,
  chainId: bigint,
  accountKey: string | undefined,
): T | undefined {
  if (!built || accountKey === undefined) return undefined;
  const same = built.chainId === chainId && built.accountKey === accountKey.toLowerCase();
...
export function useBuildWallet(session: Session): BuildWalletState {
  ...
  const wallet = usable ? currentFor(built, chainId, session.accountKey) : undefined;
  const error = usable ? currentFor(failure, chainId, session.accountKey) : undefined;
  ...
  useEffect(() => {
    ...
    void buildPool
      .run(
        buildKey(activeChain.chainId, accountKey),
        async () => {
        ...
        async (walletApi) => {
          ...
          setBuilt({
            ...
            accountKey: accountKey.toLowerCase(),
            value: walletApi,
          });
          return true;
        },
