// The vault is the only copy of a bearer key across a chain switch: its ordering guarantees matter.
...
/// `resetForTest` too: the store's parse cache and write-refused latch outlive a single test.
...
describe("vault store", () => {
  it("stores a link before it has a tx hash", () => {
    // Written before broadcast, so the key never exists only in memory.
  ...
  it("removes expired records from storage, not just from the answer", () => {
    ...
    // Idempotent, or the calling effect would re-run forever.
  ...
  it.each([
    ...
    // A bad amount would make `BigInt` throw mid-render and unmount every other link.
  ...
  it("keeps serving records this tab wrote when storage refuses the write", () => {
    // Safari private mode and a spent quota make `setItem` throw; the bearer key must not vanish.
