...
describe("link-vault storage compatibility", () => {
  /// When the legacy record was written, and the clock the cases read it at —
  /// inside its retention window however long after it the suite runs.
  ...
  it("reads records written before `copiedAt` existed", () => {
    // The key stays v1: a browser upgraded mid-flight must still see the links
    // it made last week, and those records carry no `copiedAt`.
    localStorage.setItem(STORAGE_KEY, JSON.stringify([legacy]));

    const [record] = listFor(CHAIN);
    if (!record) throw new Error("no record listed");
    expect(record.id).toBe("legacy");
    expect(record.copiedAt).toBeUndefined();
  });

  it("writes `copiedAt` under the same key, readable as before", () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify([legacy]));

    vault.markClaimLinkCopied("legacy", legacy.createdAt + 1_000);

    const stored = readStored() as Array<Record<string, unknown>>;
    expect(stored).toHaveLength(1);
    expect(stored[0]).toMatchObject({ ...legacy, copiedAt: legacy.createdAt + 1_000 });
  });

  it("rejects a `copiedAt` that is not a timestamp", () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify([{ ...legacy, copiedAt: "yesterday" }]));

    expect(listFor(CHAIN)).toEqual([]);
  });
});

describe("link-vault curation", () => {
