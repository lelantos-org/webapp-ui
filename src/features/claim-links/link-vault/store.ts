...
/// Unchanged by `copiedAt`: the field is optional, so every record written
/// before it existed still validates, and a record written after it is still a
/// v1 record to an older build (which ignores the extra key).
