...
/// an absent one means.
export interface FeeAssetOption extends AssetUnits, Pick<AssetLabel, "symbol"> {
  id: bigint;
...
/// exists.
export interface FeeShortfall extends Omit<FeeAssetOption, "affordable"> {
...
export function feeBlockFor(input: FeeBlockInputs): FeeBlock | undefined {
  ...
  const { affordable, ...short } = pays;
  return affordable
    ? undefined
    : { kind: "shortfall", ...short, alternative: alternativeTo(pays.id) };
}
