...
//
// which `useTxView` (`use-tx-view.ts`) decides and `TxOutcome` draws. What this
// owns is the frame around them, and that the fields survive all four. They are
// hidden rather than unmounted throughout, so react-hook-form keeps its state,
// "Try again" re-submits exactly what was entered, and a review's back returns
// to the form it summarised.
...
/// What a submit button says while its op is being sent: the form's CTA and the
/// review's Confirm alike.
...
export interface ActionFormProps {
  ...
  /// Which of the tx's operations was this one, when it was bundled with
  /// others — `operationOf(m.data)`. Shown on the settled card beside the hash.
