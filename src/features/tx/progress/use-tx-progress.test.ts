...
// The stepper state an op's mutation drives: which phase is current, when the
// op is done, and where a failure happened.
...
describe("useTxProgress", () => {
  ...
  it("holds a deposit open on `mined` until the relayer flushes", () => {
    ...
    // Out of the list: completes the op without moving the phase.
  ...
  it("times a successful proof, and only a successful one", () => {
    ...
    // A repeated `proving` does not restart the clock.
  ...
  it("starts clean, and reset empties it", () => {
    ...
    // A failure straight after a restart has no step to blame.
    act(() => result.current.set("failed"));
