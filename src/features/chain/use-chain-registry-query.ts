...
/// copy meanwhile.
export function useChainRegistryQuery(): UseQueryResult<ChainEntry[]> {
  ...
  return useQuery({
    ...
    // treated as cached, so the fetch still runs once on mount and the
    ...
    // require a page reload; `ChainRegistryGate`'s retry button covers the
    // remaining cases.
    retry: 2,
  });
}
