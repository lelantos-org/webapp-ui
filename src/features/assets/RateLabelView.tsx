...
import { formatWindowShort, type RateLabel, rateLabel } from "./rate-label";
...
function figureOf(label: RateLabel): { fig: string; note: string; paused: boolean } | undefined {
  ...
  if (label.kind === "paused") return { fig: "paused", note: "still fully backed", paused: true };
  return undefined;
}
...
function CompactRate({ label, asset }: { label: RateLabel; asset: RegisteredAsset }) {
  ...
  return label.kind === "unmeasured" ? (
    ...
    <span className="pf-empty__rate pf-empty__rate--none">does not earn</span>
  );
}
...
function PickerRate({ label }: { label: RateLabel }) {
  ...
  return label.kind === "unmeasured" ? (
    <span className="apick__rate apick__rate--none">
      ...
      <span className="sr-only">rate cannot be measured</span>
    </span>
  ) : (
    <span className="apick__rate apick__rate--none">does not earn</span>
  );
}
