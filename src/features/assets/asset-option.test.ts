...
const { chain, balanceOf } = vi.hoisted(() => ({
...
vi.mock("./use-balances", () => ({ useAssetBalanceLabel: () => balanceOf }));
...
describe("assetSelectOptions", () => {
  ...
  // Without the rate tag, the vault is all that separates the two.
...
describe("useAssetSelectOptions", () => {
  it("reads the registry, the balances and the chain's native adapter", () => {
    balanceOf.mockImplementation((a: { id: bigint }) => (a.id === 1n ? "2" : undefined));
