...
import { assetUsd, type PriceMap, priceOf } from "./prices";
...
/// Dollar value of a gain, which `YieldGain` keeps in the token's base units.
///
/// A gain has already been through `scale` and `index`, so it is priced as base
/// units rather than converted a second time. Signed: a venue loss prices
/// negative.
export function gainUsd(gain: bigint, decimals: number, priceUsd: number): number {
  return baseUnitsUsd(gain, decimals, priceUsd);
}
...
export function earnedTotal(
  ...
  for (const [assetId, gain] of gains) {
    ...
    usd += gainUsd(gain.gain, meta.decimals, price);
    counted++;
  }
