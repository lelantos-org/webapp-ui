...
// What a failed claim puts on the error card, and what it keeps for the log.
//
// The card renders the phase's `message` verbatim. It used to be the raw fault —
// an RPC dump or a prover error with its hex — logged under the flow's own scope
// and never classified, so a dismissed wallet prompt read as a failure too.
