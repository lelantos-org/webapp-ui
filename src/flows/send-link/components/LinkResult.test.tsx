...
import { LinkResult, maskClaimUrl } from "./LinkResult";
...
describe("LinkResult", () => {
  it("promises the vault's window, never 'until it's claimed'", () => {
    render(
      <LinkResult url={URL_} amountLabel="75 USDC" recordId="x" ttlMs={WEEK} onReset={() => {}} />,
    );
    expect(
      screen.getByText(
  ...
  it("marks the record shared once the link is copied", async () => {
    ...
    render(
      <LinkResult url={URL_} amountLabel="75 USDC" recordId={id} ttlMs={WEEK} onReset={() => {}} />,
    );
