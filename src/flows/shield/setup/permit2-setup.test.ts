...
import { ensurePermit2AuthorizedSetupBatch, type SetupProgress } from "./permit2-setup";
...
class MockChain {
  ...
  // Present only so `supportsAllowanceBatch` accepts this adapter. It now
  // also implies `supportsSigning`, since every deposit path needs an EOA.
...
function mockWallet(state: Record<string, TokenState>) {
  ...
  return { wallet: fakeWalletApi({ chain }), chain };
}
