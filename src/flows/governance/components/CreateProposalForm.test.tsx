...
describe("CreateProposalForm", () => {
  ...
  it("encodes a picked function with typed arguments and submits title + description", async () => {
    const { onSubmit } = setup();
    fireEvent.change(screen.getByLabelText("Title"), { target: { value: "Set cutoff" } });
    fireEvent.change(screen.getByLabelText("Description"), { target: { value: "One day." } });

    const row = within(action(1));
    // The governor is first, and its registered address is prefilled.
  ...
  it("switches the target when the contract changes, and takes raw calldata", async () => {
    const { onSubmit } = setup();
    fireEvent.change(screen.getByLabelText("Title"), { target: { value: "Raw" } });
    fireEvent.click(screen.getByRole("button", { name: "Add another action" }));
    ...
    // The first row stays incomplete, so nothing is submitted until it is removed.
